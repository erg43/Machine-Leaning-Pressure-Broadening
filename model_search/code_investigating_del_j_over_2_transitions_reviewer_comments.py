...
for key, data in db.items():
    ...
    if 'B0a' in data.columns:
        ...
        diffs = branch[abs(branch)>2]
        print(len(diffs))
        if len(diffs) > 0:
            dif_sum.append(list(diffs.values))
            lines.append([key, data.loc[diffs.index]])
        continue
...
#%%
linnnnes = []
for moleculesss in lines:
    molecule = moleculesss[0]
    data = moleculesss[1].reset_index()
    data['delta_J'] = data['J'] - data['Jpp']
    data = data[['nu', 'delta_J']]
    linnnnes.append([molecule, data])
#%%
for stuff in linnnnes:
    print(stuff[0])
    print(stuff[1])
#%%
linnnnes[4][1].to_csv('NH3_bad_lines', index=None)
#%%
from itertools import chain

dif_sum = list(chain.from_iterable(dif_sum))
print(dif_sum)
#%%
from collections import Counter
dif_sum = [int(i) for i in dif_sum]
Counter(dif_sum)

#%%
print(np.mean(dif_sum))
#%%
print(len(dif_sum))
#%%
print(data.columns)
