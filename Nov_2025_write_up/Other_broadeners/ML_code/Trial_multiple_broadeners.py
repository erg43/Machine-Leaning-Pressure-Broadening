...
masters_data = pd.read_csv(path+'LLM_data/LLM_scraped_data.csv', index_col=0)
...
for key, data in molecules.items():
    ...
    weight22 = (1 / weight) ** 2
    max_weight = 1000
    weight22_maxed = weight22.clip(upper=max_weight)
    # reassign weight into dictionary
    data['weight'] = weight22_maxed
    weighting_for_this_molecule = largest_data / len(data) / 100
    datas = []
