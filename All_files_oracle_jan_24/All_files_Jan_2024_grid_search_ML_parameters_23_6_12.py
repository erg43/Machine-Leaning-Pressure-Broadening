...
for key, data in db.items():
    ...
    if 'B0a' in data.columns:
        ...
        data = data[['gamma_air', 'J', 'Jpp', 'molecule_weight', 'gamma_air-err', 'm', 'findair', 
                     'molecule_dipole', 'polar', 'B0a', 'B0b', 'B0c', 'air_weight']]#, 'K', 'Ka', 'Kc', 'symmetry', 'vee']]#'molecule_weight', 'Ka_aprox', 'Kapp_aprox', 'Kc_aprox', 'Kcpp_aprox']]#, 'nu', 'sw', 'a']]
...
for key, data in molecule_list.items():
    ...
    pipe = make_pipeline(StandardScaler(), HistGradientBoostingRegressor(random_state=42))
    ...
    plot_data_list.append([key, x_plot[-1000:], y_test_plot[-1000:], y_pred[-1000:], score])#, pipe])
...
with open('hist_old_way.pkl', 'wb') as f:
